...
public class Compra {
	...
	private void fazerNovoPedido() {
		...
		FornecedorModel fornecedorSelecionado = fornecedores.stream()
				.filter(fornecedor -> fornecedor.getCnpj().equals(cnpjFornecedor)).findFirst().orElse(null);
		...
		CompraModel novoPedido = new CompraModel(LocalDate.now(), produtosQuantidades, matriculaFuncionario,
				cnpjFornecedor);
	...
	private void listarHistoricoPedidos() {
		try (Session session = neo4jConnection.getSession()) {
			Result result = session.run(
					"MATCH (c:Compra)-[contem:Contem]->(p:Produto) RETURN c.cnpjFornecedor, c.matriculaFuncionario, c.dataCompra, p.codigo, p.nome, contem.quantidade");
	...
	private void adicionarPedido(CompraModel pedido) {
		try (Session session = neo4jConnection.getSession()) {
			Result createCompra = session.run(
					"CREATE (c:Compra {cnpjFornecedor: $cnpj, matriculaFuncionario: $matricula, dataCompra: $data}) RETURN c",
			...
			session.run(
					"MATCH (f:Funcionario {matricula: $matricula}), (c:Compra {cnpjFornecedor: $cnpj, dataCompra: $data}) "
							+ "CREATE (f)-[:Realizou]->(c)",
			...
			for (Map.Entry<String, Integer> entry : pedido.getProdutosQuantidades().entrySet()) {
				...
				Result findProduct = session.run("MATCH (p:Produto {codigo: $codigo}) RETURN p",
				...
				if (findProduct.hasNext()) {
					...
					Result createProduct = session.run("CREATE (p:Produto {codigo: $codigo}) RETURN p",
							parameters("codigo", codigoProduto));
					produtoNode = createProduct.single().get("p").asNode();
				}

				session.run(
						"MATCH (c:Compra {cnpjFornecedor: $cnpj, matriculaFuncionario: $matricula, dataCompra: $data}), (p:Produto {codigo: $codigo}) "
								+ "CREATE (c)-[:Contem {quantidade: $quantidade}]->(p)",
						parameters("cnpj", pedido.getCnpjFornecedor(), "matricula", pedido.getMatriculaFuncionario(),
								"data", pedido.getDataPedido().toString(), "codigo", codigoProduto, "quantidade",
								quantidade));

				session.run(
						"MATCH (f:Fornecedor {cnpj: $cnpj}), (p:Produto {codigo: $codigo}) "
								+ "CREATE (f)-[:Fornece]->(p)",
	...
	private List<FornecedorModel> buscarTodosFornecedores() {
		try (Session session = neo4jConnection.getSession()) {
			...
			return result.stream()
					.map(record -> new FornecedorModel(record.get(0).asString(), record.get(1).asString(), null))
	...
	private List<ProdutoModel> buscarProdutosAssociados(String cnpjFornecedor) {
		try (Session session = neo4jConnection.getSession()) {
			Result result = session.run(
					"MATCH (f:Fornecedor {cnpj: $cnpj})-[:Fornece]->(p:Produto) RETURN DISTINCT p.codigo, p.nome, p.precocompra, p.precovenda",
					parameters("cnpj", cnpjFornecedor));

			return result.stream()
					.map(record -> new ProdutoModel(record.get("p.codigo").asString(), record.get("p.nome").asString(),
							record.get("p.precocompra").asLong(), record.get("p.precovenda").asLong()))
					.collect(Collectors.toList());
		} catch (Exception e) {
