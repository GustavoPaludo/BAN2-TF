...
public class Fornecedor {
    ...
    private void listarFornecedores() {
        ...
        for (FornecedorModel fornecedor : fornecedores) {
            System.out.println(fornecedor.getCnpj() + " - " + fornecedor.getNome() + " - Produtos: " + fornecedor.getProdutos());
        }
    }
    ...
    public void adicionarFornecedor(FornecedorModel fornecedor) {
        try (Session session = neo4jConnection.getSession()) {
            List<String> produtosCodigos = fornecedor.getProdutos().stream()
                    .map(ProdutoModel::getCodigo)
                    .collect(Collectors.toList());

            Result result = session.run("CREATE (f:Fornecedor {cnpj: $cnpj, nome: $nome, produtos: $produtos}) "
                            + "WITH f UNWIND $produtos AS produtoCodigo "
                            + "MATCH (p:Produto {codigo: produtoCodigo}) "
                            + "MERGE (f)-[:Fornece]->(p)",
                    parameters("cnpj", fornecedor.getCnpj(), "nome", fornecedor.getNome(), "produtos", produtosCodigos));
    ...
    public void editarFornecedor(FornecedorModel fornecedor) {
        try (Session session = neo4jConnection.getSession()) {
            ...
            session.run("MATCH (f:Fornecedor {cnpj: $cnpj}) SET f.nome = $nome, f.produtos = $produtos",
                    parameters("cnpj", fornecedor.getCnpj(), "nome", fornecedor.getNome(), "produtos", produtosCodigos));
    ...
    public void excluirFornecedor(String cnpj) {
        try (Session session = neo4jConnection.getSession()) {
            Result result = session.run("MATCH (f:Fornecedor {cnpj: $cnpj}) DETACH DELETE f",
    ...
    public List<FornecedorModel> buscarTodosFornecedores() {
        try (Session session = neo4jConnection.getSession()) {
            ...
            return result.stream().map(record -> {
                ...
                List<ProdutoModel> produtos = produtosCodigos.stream()
                        .map(this::buscarProdutoPorCodigo)
